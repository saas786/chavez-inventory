<template>
	<v-dialog
		max-width="1200"
		@click:outside="$refs.createCatalogItemForm.reset()"
	>
		<template v-slot:activator="{ on, attrs }">
			<v-btn text color="primary" v-bind="attrs" v-on="on">Add Item</v-btn>
		</template>
		<v-form ref="createCatalogItemForm">
			<v-card>
				...
				<v-card-actions class="justify-center">
					<v-btn color="success" outlined @click="saveItem">Save</v-btn>
				</v-card-actions>
			</v-card>
		</v-form>
	</v-dialog>
...
export default {
	...
	methods: {
		...
		saveItem() {
			...
			Inertia.post("/inventory/catalog/", data);
		},
